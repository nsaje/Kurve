...
implements CommandListener
{
    ...
    public BluetoothDiscovery( Display disp )
    {
        ...
        {   // not much that can be done
            ...
            String message = "Neka napaka s tvojim modrim zobom: " +
                             e.getMessage();
    ...
    private void closeNotifier()
    {
        synchronized( block_notifier )
        {
            if( notifier != null )
            {
                ...
                {
                    String message = "Neka napaka z modrim zobom (notifier)" +
                                     e.getMessage();
                    ErrorScreen.showError(message, display.getCurrent());
                }
                notifier = null;
        ...
        switch( c.getCommandType() )
        {
            case Command.CANCEL:
                ...
                try
                {
                    ...
                    {   // cancel client
                        // Info note to user
                        Alert a = new Alert( "", "Iskanje prekinjeno", null, AlertType.INFO );
                ...
                {
                    String message = "Napaka med prekinjanjem: " +
                                     e.getMessage();
                    ErrorScreen.showError(message, display.getCurrent());
                }
                break;
        ...
        throws BluetoothStateException, InterruptedException, IOException
    {
        ...
        progressBar = new InqProgressBar( "Iščem telefone...", 105 );
        ...
        {   // connect only if devices have been found
            ...
            progressBar = new PageProgressBar( "Konektam...", urlStrings.size() );
            // Connect all devices
            ...
            {   // Retrieve remote name
                ...
                if (rname==null || rname=="") rname = "Neznanec";
                // Update progress bar
    ...
    throws BluetoothStateException, IOException, InterruptedException
    {
        ...
        setTitle( "Čakam" );
        setString( "Čakam, da se kdo konekta... SAFR" );
        setTimeout( FOREVER );
        addCommand( new Command( "Prekliči", Command.CANCEL, 1 ) );
    ...
    extends TimerTask
    {
        ...
        private InqProgressBar( String title, int max )
        {
            ...
            Command cmStop = new Command( "Prekliči", Command.CANCEL, 1 );
    ...
    implements DiscoveryListener
    {
        ...
        public void inquiryCompleted( int discType )
        {
            ...
            {   // Check if devices have been found
                ...
                {   // No device found
                    warning = "Nisem najdu nobenga telefona";
                }
                else
                {   // Stop Inquiry progress bar
                    progressBar.stop();
                    // Start service search progress bar
                    progressBar = new PageProgressBar( "Iščem Kurve...", cached_devices.size() );
    ...
    implements CommandListener
    {
        ...
        public DeviceList( int list_type )
        {
            ...
            cancel = new Command( "Prekliči", Command.CANCEL, 1 );
            // append commands
            addCommand( ok );
            addCommand( stop );
            setCommandListener( this );
